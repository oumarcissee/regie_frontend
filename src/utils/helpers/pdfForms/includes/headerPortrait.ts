 ...
 */
const headerPortrait = (doc: jsPDF, title: string, currentDate: string, name?: string) => {
    ...
    // Month display with enhanced styling and text wrapping
    if (name) {
        ...
        if (nameWidth > maxWidth) {
            // Text needs wrapping
            const splitText = doc.splitTextToSize(name, maxWidth);
            doc.text(splitText, pageWidth / 2, 2, { align: 'center' });
        } else {
            // Text fits on one line
            doc.text(name, pageWidth / 2, 2, { align: 'center' });
        }
    }
